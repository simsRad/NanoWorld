# PowerShell script to add Office 365 Users data source with proper environment handling
param(
    [string]$ApiId = "shared_office365users",
    [string]$ConnectionId = "4361a4de3ed842908c159e1023fe34b9"
)
...
# Try to add the Office 365 Users data source
Write-Host "Adding Office 365 Users data source..." -ForegroundColor Green
Write-Host "Command: pac code add-data-source -a `"$ApiId`" -c `"$ConnectionId`"" -ForegroundColor Cyan

try {
    & pac code add-data-source -a $ApiId -c $ConnectionId
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Office 365 Users data source added successfully!" -ForegroundColor Green
    } else {
